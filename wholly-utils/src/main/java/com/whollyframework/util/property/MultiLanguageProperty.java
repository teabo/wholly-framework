...
//	private static MultiLanguageProcess mp = null;
	private static Map<Object, Object> propCache = null;
	private static Map<Object, Object> defaultProps = null;
	...
	public static synchronized void init() throws Exception {
		defaultProps = new HashMap<Object, Object>();
	...
	public static synchronized void load(String application, boolean clearCache) throws Exception {
		...
		if (propCache != null && !clearCache) {
			prop = (Properties) propCache.get(application);
		} else {
			if (propCache == null)
				propCache = new HashMap<Object, Object>();
...
//			}
//			if (application != null) {
//				Collection<?> cols = mp.simpleQuery(null, application);
//				if (cols != null) {
//					for (Iterator<?> iterator = cols.iterator(); iterator.hasNext();) {
//						MultiLanguage language = (MultiLanguage) iterator.next();
...
//		}
	...
	public static String getProperty(String application, String language, String key, String defaultValue) throws Exception {
		...
		Properties p = (Properties) propCache.get(application);
		String value = null;
		if (p == null || p.isEmpty()) {
	...
	public static String getProperty(String language, String key, String defaultValue) throws Exception {
		...
		Properties prop = (Properties) defaultProps.get(language);
		String rtn = null;
		if (prop != null)
			rtn = prop.getProperty(key, defaultValue);
		else {
			init(language);
			prop = (Properties) defaultProps.get(language);
			rtn = prop.getProperty(key, defaultValue);
		}
	...
	private static void init(String language) throws IOException {
		...
		URL propsUrl = Thread.currentThread().getContextClassLoader().getResource(
				"i18n" + File.separator + "multiLanguage_i18n_" + language + ".properties");
	...
	public static String getProperty(String key, String defaultValue) throws Exception {
		Properties prop = (Properties) defaultProps.get(NAMES[0]);
