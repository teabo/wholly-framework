...
public class IPPoolUtil {
	public static void main(String[] args) {
		...
		int netMask = getNetMask("255.255.255.0");
		System.out.println(netMask);

		Nets nets = getIPSection("10.229.0.1", netMask);
		System.out.println(nets.getStartIP());
		System.out.println(nets.getEndIP());
		System.out.println(nets.getIPSection());
		System.out.println(ipIsInNet(nets.getIPSection(), "10.229.0.111"));
	 ...
	 * 根据起始IP地址和子网掩码计算终止IP
	 */
	public static Nets getIPSection(String StartIP, int netmask) {
		return getIPSection(StartIP, getMask(netmask));
	}

	/**
	 * 根据起始IP地址和子网掩码计算终止IP
	 */
	public static Nets getIPSection(String StartIP, String netmask) {
		Nets nets = new Nets();
...
class Nets {
	...
	public String getIPSection() {
		if (IPSection == null) {
			...
			buf.append(getStartIP()).append("-");
			buf.append(getEndIP());
			IPSection = buf.toString();
		}
		return IPSection;
	}
}