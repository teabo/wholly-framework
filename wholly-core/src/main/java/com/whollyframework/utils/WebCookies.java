...
public class WebCookies implements Serializable {
	...
	public String getValue(String name) {
		...
		if (cookie != null) {
			if (Security.ENCRYPTION_BASE64.equals(encryption)) {
				return Security.decodeBASE64(cookie.getValue());
			}
	...
	public void addCookie(String name, String value, int maxAge) {
		...
		if (null != response) {
			Cookie cookie = null;
			if (Security.ENCRYPTION_BASE64.equals(encryption)) {
				cookie = new Cookie(name, Security.encodeToBASE64(value));// BASE64
			} else {
