...
public class HibernateSupportDAO<T, PK extends Serializable> {
	 ...
	 */
	public HibernateSupportDAO(final SessionFactory sessionFactory, final Class<T> entityClass) {
	...
	public void delete(final PK[] ids){
	    ...
	    hql.append("Delete ").append(entityClass.getSimpleName());
        ...
        if (ids.length > 1) {
            hql.append(" where ").append(getIdName()).append(" in (").append(idsbuffer).append(")");
        } else {
            hql.append(" where ").append(getIdName()).append(" = ").append(idsbuffer);
        }
        executeUpdate(hql.toString(), values);
	}
	
	protected int executeUpdate(String hql, List<Object> values) {
