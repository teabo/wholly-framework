    ...
    if(isFormat==false){
    	...
    	for(var i=0;i<columns[0].length;i++){
    		for(var j=0;j<rows.length;j++){
    			if (dataRows[j]== undefined) dataRows[j] = {};
    			if(columns[0][i].formatter){
    				dataRows[j][columns[0][i].field]=format(rows[j],columns[0][i],j);
            	} else {
            		dataRows[j][columns[0][i].field]=rows[j][columns[0][i].field];
            	}
        	}
        }
    }
    var dataGrid_datas = JSON.stringify(dataRows);
...
function removeCheckBox( array){
	...
	for ( var i = 0; i < array.length; i++) {
		if( array[i].field.toString()=="userState"){
			array.remove(i);
		}
	}
}
