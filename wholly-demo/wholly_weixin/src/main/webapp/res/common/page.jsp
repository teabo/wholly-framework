...
<c:choose>
	<c:when test="${param.app_id != null }">
<input type="hidden" name="app_id" value='${param.app_id }' />
	</c:when>
	<c:when test="${content.app_id != null }">
<input type="hidden" name="app_id" value='${content.app_id }' />
	</c:when>
</c:choose> --%>
</div>
...
<!--  <input type="hidden" id="istemp" name="istemp" value='${content.istemp }' />-->
<%-- <input type="hidden" id="version" name="version" value='${content.version }' /> --%>
<!--起草人ID -->
<input type="hidden" id="authorid" name="authorId" value="${content.authorId}" />
<!--起草人名称 -->
<input type="hidden" id="author" name="author" value="${content.author}" />
<!--起草时间 -->
<input type="hidden" id="created" name="created" value="${content.createdStr}" />