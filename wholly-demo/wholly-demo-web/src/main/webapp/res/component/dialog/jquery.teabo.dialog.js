...
$.fn.Dialog = (function() {
	...
	IMGFOLDERPATH=contextPath + '/res/component/dialog/images/',
	CONTEXT_PATH = '',
	DIALOG_OPTIONS = {
		ID : "dialog0001",
		isModal : true,
	...
	Dialog.extend({
		...
		show : function(configs) {
			...
			var bgdiv = pw.EDialog.DM("_DialogBGDiv");
			if(!bgdiv){
				bgdiv = pw.document.createElement("div");
				bgdiv.id = "_DialogBGDiv";
				this.hideE(bgdiv);
			 	pw.EDialog.TG("body")[0].appendChild(bgdiv);
				if(isIE6){
			...
			var div = pw.EDialog.DM("_DialogDiv_"+dlg_id);
			if(!div){
				...
				// div.setAttribute("dragStart","EDialog.dragStart");
			 	pw.EDialog.TG("body")[0].appendChild(div);
			}
			...
			if(options.innerElementId){
				...
				if(isIE){
					...
					pw.EDialog.DM("_Covering_"+dlg_id).parentNode.appendChild(fragment);
				}else{
					pw.EDialog.DM("_Covering_"+dlg_id).parentNode.appendChild(innerElement);
				}
			}
			pw.EDialog.DM("_DialogDiv_"+dlg_id).DialogInstance = this;
			if(options.url)
				pw.EDialog.DM("_DialogFrame_"+dlg_id).DialogInstance = this;
			Drag.init(pw.EDialog.DM("_draghandle_"+dlg_id),pw.EDialog.DM("_DialogDiv_"+dlg_id));// 注册拖拽方法
			if(!isIE){
				pw.EDialog.DM("_DialogDiv_"+dlg_id).dialogId=dlg_id;
				pw.EDialog.DM("_DialogDiv_"+dlg_id).onDragStart = function(){
					EDialog.DM("_Covering_"+this.dialogId).style.display="";
				};
				pw.EDialog.DM("_DialogDiv_"+dlg_id).onDragEnd = function(){
					EDialog.DM("_Covering_"+this.dialogId).style.display="none";
				};
			}

			OKButton = pw.EDialog.DM("_ButtonOK_"+dlg_id);
			CancelButton = pw.EDialog.DM("_ButtonCancel_"+dlg_id);

			// 显示标题图片
			if(options.showMessage){
				this.showE(pw.EDialog.DM("_MessageRow_"+dlg_id));
				if(options.messageTitle){
					pw.EDialog.DM("_MessageTitle_"+dlg_id).innerHTML = options.messageTitle;
				}
				if(options.message){
					pw.EDialog.DM("_Message_"+dlg_id).innerHTML = options.message;
				}
			}
			
			if(!options.AlertFlag){
				this.showE(bgdiv);
				options.bgdivID = "_DialogBGDiv";
			}else{
				bgdiv = pw.EDialog.DM("_AlertBGDiv");
				if(!bgdiv){
					bgdiv = pw.document.createElement("div");
			...
			if(!options.showButton){
				this.hideE(pw.EDialog.DM("_ButtonRow_"+dlg_id));
			} else {
				OKButton.focus();
			}
			...
			pw.EDialog.pushArray(this);// 放入队列中，以便于ESC时正确关闭
		},
		...
		close : function(){
			...
			if(options.innerElementId){
				...
				if(isIE){
					...
					fragment.innerHTML=innerElement.outerHTML;
					innerElement.outerHTML="";
					this.TG("body")[0].appendChild(fragment);
				}else{
					this.TG("body")[0].appendChild(innerElement);
				}
		...
		getInstance : function(id){
			var pw = EDialog.getTopLevelWindow();
			var f = pw.EDialog.DM("_DialogDiv_"+id);
			if(!f){
				return null;
			}
	...
	Array.prototype.removeDialog = function(s){
		for(var i=0;i<this.length;i++){
			...
			if(dlg && s == dlg.getId()){
				this.splice(i, 1);
				window.top.result = dlg.getResult();
			}
		}
	};
	Array.prototype.getDialog = function(s){
			...
			if(d){
				try{
					...
					if(!d.getOptions().AlertFlag){
						...
						if(!flag){
							EDialog.DM("_DialogBGDiv").style.opacity="0.4";
							EDialog.DM("_DialogBGDiv").style.filter="alpha(opacity=40)";
						}
					}
					if(d.getOptions().AlertFlag){
		...
		Dialog.onKeyDown = function(event){
			if(event.shiftKey&&event.keyCode==9){//shift键
				var pw =window.top;
				if(pw.EDialog.size()>0){
					stopEvent(event);
					return false;
				}
			}
			if(event.keyCode==27){//ESC键
				var DialogArr=window.top.EDialog.getArray();
		...
		Dialog.prototype.setPosition=function(){
			...
			pw.EDialog.DM(this.options.bgdivID).style.height= sh + "px";
		};
