...
public class SpyMemcachedManager {
	...
	private SpyMemcachedServer server;

	private MemcachedClient memClient;

	public SpyMemcachedManager(SpyMemcachedServer server) {
		this.server = server;
	}
	...
	public void connect() throws IOException {
		...
		buf.append(server.getIp()).append(":").append(server.getPort());
		
		if (StringUtil.isBlank(server.getUsername())){
			memClient = new MemcachedClient(AddrUtil.getAddresses(buf.toString()));
		} else {
			AuthDescriptor ad = new AuthDescriptor(new String[]{"PLAIN"}, new PlainCallbackHandler(server.getUsername(), server.getPassword()));
