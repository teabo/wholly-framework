...
import com.whollyframework.ddltools.alteration.ColumnDataTransferChange;
...
import com.whollyframework.ddltools.alteration.TableDataTransferChange;
...
public abstract class AbstractTableDefinition {
	protected static Logger log = Logger
			.getLogger(AbstractTableDefinition.class);
	...
	public void processChanges(ChangeLog changeLog) throws Exception {
		...
		for (Iterator<ModelChange> iterator = changes.iterator(); iterator
				.hasNext();) {
	...
	public void processChange(ColumnDataTransferChange change)
			throws SQLException {
		StringBuffer buffer = new StringBuffer();
		getSQLBuilder().setWriter(buffer);
		getSQLBuilder().columnDataCopy(change.getTable(),
				change.getSourceColumn(), change.getTargetColumn());
	...
	public void processChange(ColumnDataTypeChange change) throws SQLException {
		...
		if (targetColumn.getName().equalsIgnoreCase(sourceColumn.getName())) {
			getSQLBuilder().modifyColumnDataType(table, targetColumn);
		} else {
			...
			try {
				...
				tempColumn.setName(sourceColumn.getName());
	...
	public void processChange(ColumnRenameChange change) throws SQLException {
		StringBuffer buffer = new StringBuffer();
		getSQLBuilder().setWriter(buffer);
		getSQLBuilder().columnRename(change.getTable(),
				change.getSourceColumn(), change.getTargetColumn());
	...
	public void processChange(TableDataTransferChange change)
			throws SQLException {
		StringBuffer buffer = new StringBuffer();
		getSQLBuilder().setWriter(buffer);
		getSQLBuilder().tableDataCopy(change.getSourceTable(),
	...
	public void processChange(TableRenameChange change) throws SQLException {
		...
		// getSQLBuilder().tableDataCopy(change.getChangedTable(),
		// change.getTargetTable());
		getSQLBuilder().createTableAsSelect(change.getTargetTable(),
				change.getChangedTable(), false);
