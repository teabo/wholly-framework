...
public class StringList {
	...
	public void replace(String from, String to) {
		for (int i = 0; i < context.size(); i++) {
			if (from.equals((String) context.get(i))) {
				context.set(i, to);
			}
		}
	}
	...
	public void sort(boolean desc) {
		...
		if (len > 1) {
			...
			for (i = 0; i < len - 1; i++) {
				for (j = i + 1; j < len; j++) {
					if (desc) {
						if (((String) context.get(i))
								.compareTo(((String) context.get(j))) < 0) {
							strTmp = (String) context.get(i);
	...
	public void trim() {
		for (int i = 0; i < context.size(); i++) {
			if (((String) context.get(i)).equals("")) {
				context.remove(i);
			}
		}
	}
